...
function customLabel(rule){
  if(rule.freq==='monthly'){
    const ds=(rule.monthDays||[]).slice().sort((a,b)=>a-b);
    return 'Monthly \u00b7 '+ds.map(d=>d+ordinalSuffix(d)).join(', ');
  }
  const ds=(rule.days||[]).slice().sort();
  if(ds.length===5 && [1,2,3,4,5].every(w=>ds.includes(w))) return 'Weekdays';
  return ds.map(w=>['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][w]).join(' & ');
}
...
function syncRecurringActivities(activities,team,customRules){
  ...
  const custom=(customRules||[]).filter(Boolean);
  const customSourceIds=new Set(custom.map(r=>r.sourceId));
  // The activity a custom rule was built from becomes the rule's first occurrence, so it
  // must not also survive as a standalone card.
  const nonRecurring=activities.filter(a=>!a.recurring && !customSourceIds.has(a.id));
  const existingById={};
  ...
  for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){
    ...
    const matches=[];
    RECURRING_RULES.forEach(rule=>{if(rule.days.includes(wd)) matches.push({rule,label:labelFor(rule),participants:defaultTeam});});
    custom.forEach(rule=>{
      const hit=rule.freq==='monthly'?(rule.monthDays||[]).includes(dom):(rule.days||[]).includes(wd);
      if(hit) matches.push({rule,label:customLabel(rule),participants:rule.participants&&rule.participants.length?rule.participants:defaultTeam});
    });
    matches.forEach(({rule,label,participants})=>{
      const id='rec-'+rule.key+'-'+dateIso;
      if(existingById[id]){
        const ex=existingById[id];
        // Upcoming recurring occurrences always reflect the live team so nobody added
        // later is missing from them; past occurrences keep their historical roster.
        const parts=dateIso>=isoLocal(today)
          ? Array.from(new Set([...(ex.participants||[]),...participants]))
          : (ex.participants||[]);
        out.push({...ex,participants:parts,category:ex.category||rule.category||'',recurringLabel:label});
        return;
      }
      out.push({
        id,title:rule.title,description:rule.description,date:dateIso,time:rule.time,
        status:'planned',category:rule.category||'',participants,claimedBy:null,claimedAt:null,
        recurring:true,recurringLabel:label,
        tasks:(rule.deliverables||[]).map((label2,i)=>({id:id+'-t'+i,title:label2,assignee:'',deadline:dateIso,deadlineTime:rule.time,done:false}))
      });
    });
  }
